...
- **Side-by-side strategy comparison** on the same article (Compare strategies tab).
...
- **Per-summary API metrics**: word counts, compression ratio, and latency.
- **Estimated ROUGE bars in the UI** (validation-set benchmark averages per strategy; not computed live per article).
...
├── RefactoredNewsContentSummarization.ipynb    # Training notebook (Colab/local; optional in repo)
...
> large, generated artifacts and are git-ignored. They are produced by
> `RefactoredNewsContentSummarization.ipynb` (see below).
...
`RefactoredNewsContentSummarization.ipynb`, which fine-tunes the T5 summarizer,
trains the Word2Vec model, and writes the search corpus. Running all cells
produces:
...
1. **Training (`RefactoredNewsContentSummarization.ipynb`)** loads the BBC News
   Summary dataset, fine-tunes `T5-small` on the `summarize:` task, trains a
   Word2Vec model on preprocessed article tokens, and writes a flattened
   one-article-per-line corpus.
...
- Running the **training notebook** downloads the BBC dataset from Hugging Face
  the first time.
- Running **`app.py`** downloads required NLTK data (`punkt`, `punkt_tab`,
  `stopwords`) automatically on first use.
...
- ROUGE bars shown in the Summarize tab are **estimated benchmark scores** from
  validation experiments (per strategy), not live scores for the pasted article.
